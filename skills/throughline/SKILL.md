---
name: throughline
description: Keep a coding agent on its original objective across context compaction, for both Codex and Claude Code. Use at the start of any multi-step task that may run long enough to trigger compaction (refactors, feature builds, migrations, multi-file changes, long autonomous runs), and whenever a resumed session has narrowed the objective to "harden/validate/clean up existing code" instead of finishing the original goal. Also use when the user asks to set up anti-drift, an objective card, a compaction safeguard, or to install the throughline hooks.
---

# Throughline

## Overview
Compaction replaces the live transcript with a summary, and long tasks lose the original
objective to it. This skill keeps an on-disk objective card as the single source of truth,
forces compaction summaries to preserve the objective verbatim, and re-injects the card on
manual turns and resume. The on-disk card is the core; the tool-specific hooks are
reinforcement. See [mechanics.md](references/mechanics.md) for why drift happens and the
honest limits.
...
narrow it. List OUT OF SCOPE, the milestone checklist, and the next action.

### 2. Keep it bounded (token discipline)
Overwrite in place; never append-grow. Respect the size budget in the template header:
milestones not micro-steps, decisions log capped at the last 10 lines, objective stored once.
A bloated card is just another context leak.

### 3. Work the card at every milestone
Before starting a milestone, re-read the card. After finishing one, update the checklist and
NEXT ACTION and save. If current work no longer serves the objective, write `DRIFT?: <why>`
in NEXT ACTION and correct course before continuing.
...
- Codex: add `experimental_compact_prompt_file` (the verbatim objective-lock) and `hooks = "./hooks.json"` to `config.toml`. See [codex-setup.md](references/codex-setup.md).
...
firing. If compaction storms persist, reduce noisy tool output and split to a fresh thread at
a milestone, carrying the card forward for a lossless handoff.
